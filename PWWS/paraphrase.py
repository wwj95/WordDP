...
def _synonym_prefilter_fn_v1(token, synonym):
    '''
    Similarity heuristics go here
    '''
    if (len(synonym.text.split()) > 2   # the synonym produced is a phrase
            or 
            (synonym.lemma == token.lemma)   # token and synonym are the same
            #or 
            #(synonym.tag != token.tag)   # the pos of the token synonyms are different
            or 
            (token.text.lower() == 'be')  # token is be
        ): 
        return False
...
def _generate_synonym_candidates_v1(token, token_position, rank_fn=None):
    '''
    Generate synonym candidates.
    For each token in the doc, the list of WordNet synonyms is expanded.
    :return candidates, a list, whose type of element is <class '__main__.SubstitutionCandidate'>
