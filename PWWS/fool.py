...
from PWWS.adversarial_tools import ForwardGradWrapper, adversarial_paraphrase
...
def fool_text_classifier():
    ...
    scores_origin = model.evaluate(x_test[args.start:args.start+clean_samples_cap], y_test[args.start:args.start+clean_samples_cap])
    ...
    classes_prediction = grad_guide.predict_classes(x_test[args.start:args.start+clean_samples_cap])
    ...
    adv_text_path = r'./fool_result/{}/{}/adv_{}_new.txt'.format(dataset, args.model, str(clean_samples_cap))
    change_tuple_path = r'./fool_result/{}/{}/change_tuple_{}_new.txt'.format(dataset, args.model, str(clean_samples_cap))
    file_1 = open(adv_text_path, "a")
    file_2 = open(change_tuple_path, "a")
    for index, text in enumerate(test_texts[args.start:args.start+clean_samples_cap]):
        sub_rate = 0
        NE_rate = 0
        if np.argmax(y_test[index+args.start]) == classes_prediction[index]:
            ...
            adv_doc, adv_y, sub_rate, NE_rate, change_tuple_list = adversarial_paraphrase(input_text=text,
                                                                                          true_y=np.argmax(y_test[index+args.start]),
                                                                                          grad_guide=grad_guide,
                                                                                          tokenizer=tokenizer,
                                                                                          dataset=dataset,
                                                                                          level=args.level)
            if adv_y != np.argmax(y_test[index+args.start]):
                successful_perturbations += 1
                print('{}. Successful example crafted.'.format(index+args.start))
            else:
                failed_perturbations += 1
                print('{}. Failure.'.format(index+args.start))

            text = adv_doc
            sub_rate_list.append(sub_rate)
            NE_rate_list.append(NE_rate)
            file_2.write(str(index+args.start) + str(change_tuple_list) + '\n')
